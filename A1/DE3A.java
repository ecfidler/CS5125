// DE3A.java CS5125/6025 cheng 2023
...
//  or java DE3A and then enter message
...
public class DE3A {
  ...
  void padding() { // multiple of 512 bits or 64 bytes
    ...
    for (int i = 0; i < remainder + 4; i++) buffer[messageLength + 1 + i] = 0;
  ...
  void message2words(int chunkBegins) {
    for (int i = 0; i < 16; i++) words[i] = bytes2word(chunkBegins + i * 4);
    for (int i = 16; i < 64; i++) {
      int s0 =
        Integer.rotateRight(words[i - 15], 7) ^
        Integer.rotateRight(words[i - 15], 18) ^
        words[i - 15] >>> 3;
      int s1 =
        Integer.rotateRight(words[i - 2], 17) ^
