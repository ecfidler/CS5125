// DE9D.java CS5125/6025 Cheng 2023
// Implementing AES decryption with CFB mode, IV = 0, s = 8
// Usage: java DE9D key < DE9Dtest > original.txt
...
public class DE9D {
  ...
  void makeLog() {
    alog[0] = 1;
    for (int i = 1; i < fieldSize; i++) alog[i] =
      modMultiply(logBase, alog[i - 1], irreducible);
  ...
  void buildS() {
    ...
    for (int i = 0; i < fieldSize; i++) {
      ...
      for (int k = 0; k < 8; k++) bitColumn[k] = inverse >> (7 - k) & 1;
      S[i] = 0;
      for (int k = 0; k < 8; k++) {
        int bit = B[k];
        for (int l = 0; l < 8; l++) if (bitColumn[l] == 1) bit ^= A[k][l];
        S[i] ^= bit << 7 - k;
      }
    }
  }
  ...
  int readBlock() {
    ...
    if (len <= 0) return len;
    for (int i = 0; i < len; i++) {
      if (data[i] < 0) inBlock[i] = data[i] + fieldSize; else inBlock[i] =
        data[i];
    }
    return len;
  }
  ...
  void mixColumns() {
    ...
    for (int k = 0; k < 4; k++) {
      for (int i = 0; i < 4; i++) {
        temp[i] = 0;
        for (int j = 0; j < 4; j++) temp[i] ^=
          logMultiply(G[j][i], state[k * 4 + j]);
      }
  ...
  void expandKey() {
    ...
    for (int i = 1; i < numberOfRounds; i++) {
      ...
      rcon <<= 1;
      if (rcon > 0xFF) rcon ^= irreducible;
      roundKey[i][1] = S[roundKey[i - 1][14]];
      roundKey[i][2] = S[roundKey[i - 1][15]];
      roundKey[i][3] = S[roundKey[i - 1][12]];
      for (int k = 0; k < 4; k++) roundKey[i][k] ^= roundKey[i - 1][k];
      for (int k = 4; k < blockSize; k++) roundKey[i][k] =
        roundKey[i][k - 4] ^ roundKey[i - 1][k];
    }
  }

  void addRoundKey(int round) {
    for (int k = 0; k < blockSize; k++) state[k] ^= roundKey[round][k];
  }
  ...
  void writeBlock(int len) {
    byte[] data = new byte[blockSize];
    for (int i = 0; i < len; i++) data[i] = (byte) (outBlock[i]);
    System.out.write(data, 0, len);
  }

  void shiftRegister(int newValue) {
    for (int i = 1; i < blockSize; i++) register[i - 1] = register[i];
    register[blockSize - 1] = newValue;
  }

  void copyBlock(int[] destination, int[] source) {
    for (int k = 0; k < blockSize; k++) destination[k] = source[k];
  }
  ...
  public void decrypt() {
    ...
    while ((len = readBlock()) >= 0) {
      for (int i = 0; i < len; i++) {
        // Your code to reverse DE9C.encrypt() lines
  ...
  public static void main(String[] args) {
    if (args.length < 1) {
      System.err.println("Usage: java DE9D key < DE9Dtest > original.txt");
      return;
    }
    DE9D de9 = new DE9D();
    de9.makeLog();
