// DE9A.java CS5125/6025 Cheng 2023
// Implementing AES encryption with CBC mode, IV = 0
// Usage: java DE9A < original > encrypted
...
public class DE9A {
  ...
  int readBlock() {
    ...
    if (len <= 0) return len;
    for (int i = 0; i < len; i++) {
      if (data[i] < 0) state[i] = data[i] + fieldSize; else state[i] = data[i];
    }
  ...
  void writeBlock() {
    byte[] data = new byte[blockSize];
    for (int i = 0; i < blockSize; i++) data[i] = (byte) (state[i]);
    try {
